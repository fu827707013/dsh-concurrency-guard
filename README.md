...
- 📈 **持久化统计**：按天汇总（请求/完成/异常/**中断**/取消/门闩/fail-open）+ **异常分类聚合**
  （限流/超时/网络/中断/鉴权/上游/其他）+ **异常明细**（按错误信息聚类：相同异常聚合计数、
  含错误码与最近出现时间，URL/时间戳差异自动归一化合并）+ 来源分布；随状态文件落盘，
  **重启不丢**——面板「今日统计」「每日历史」与「异常明细」跨进程连续累计。
...
finish-error chunk 识别（DSH 不抛异常的请求失败）/ 异常明细聚合（按信息聚类计数 + 错误码 + 重启保留）。
