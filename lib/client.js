...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const SESSIONS_URL = "/api/concurrency-guard/sessions";
		const POLL_MS = 1500;
		...
		const CFG_FIELDS = [
			...
			{ key: "dayRetention", label: "历史保留天数", unit: "天", desc: "只保留最近 N 天的每日汇总/异常聚合/逐条事件（跨天自动裁剪）；0 = 关闭（全量保留）" },
		...
		const cfgCheck = (label, desc, value, onChange) =>
			h("div", { style: { marginBottom: "14px", display: "flex", gap: "10px", alignItems: "flex-start" } },
				h("input", {
					type: "checkbox",
					checked: !!value,
					onChange: (e) => onChange(e.target.checked),
					style: { marginTop: "2px", accentColor: "var(--dsw-state-business-primary, #3b82f6)" }
				}),
				h("div", { style: { flex: 1 } },
					h("div", { style: { fontSize: "13px", color: V.text, fontWeight: 600, marginBottom: "3px" } }, label),
					h("div", { style: { fontSize: "11px", color: V.dim, lineHeight: "1.5" } }, desc)
				)
			);
		...
		function ConcurrencyPanel() {
			...
			const [errTab, setErrTab] = useState("kind");
			const [errScope, setErrScope] = useState("today");
			const [showCfg, setShowCfg] = useState(false);
			const [showClean, setShowClean] = useState(false);
			const [cleanDraft, setCleanDraft] = useState({ keepDays: 3, scope: { days: true, details: true, events: true, totals: true } });
			...
			const patchHistory = useCallback(async (action, extra = {}) => {
				if (busy) return;
				setBusy(true);
				try {
			...
			const patchSession = useCallback(async (action, sessionId, cap) => {
				if (busy) return;
				setBusy(true);
				try {
					const body = { action, sessionId };
					if (action === "set") body.cap = Math.max(0, Math.min(64, Math.floor(Number(cap) || 0)));
					...
					if (!res.ok) throw new Error(json?.error || `HTTP ${res.status}`);
					poll();
					// 操作成功后清掉该行草稿值（避免输入框残留已应用的数字）
					setSessionDraft((d) => { const next = { ...d }; delete next[sessionId]; return next; });
				} catch (e) {
					setError(`会话并发调整失败：${e.message || e}`);
				} finally {
					if (mounted.current) setBusy(false);
				}
			}, [busy, poll]);
			...
			// ── 报表②：按 分类 × 错误信息 汇总——数据源 errorDetails（全历史聚合）──
			// 每行 = 一类错误信息（含分类、次数、完整错误信息），按分类分组、组内次数降序，
			// 比 ①会话×错误 更适合"扫一眼哪种错误最多"。
			const errKindRows = (() => {
				const buildInfo = (d, key) => {
					let info = d.message ?? key;
					if (d.code && !info.includes(`(code=${d.code})`)) info = `${info} (code=${d.code})`;
					const metaBits = [];
					if (d.status) metaBits.push(`HTTP ${d.status}`);
				...
				const rows = [];
				for (const [key, d] of Object.entries(st?.errorDetails ?? {})) {
					rows.push({ kind: d.kind ?? "unknown", info: buildInfo(d, key), count: d.count ?? 1, lastAt: d.lastAt ?? 0 });
				}
				// 分类固定顺序，组内按次数降序、其次最近出现降序
				const kindOrder = { rate_limit: 0, timeout: 1, network: 2, provider: 3, auth: 4, aborted: 5, other: 6, unknown: 7 };
				return rows
					.sort((a, b) =>
					...
					.slice(0, 100)
					.map((r) => [
						{ text: ERR_LABEL[r.kind] ?? r.kind ?? "未知", color: r.kind === "rate_limit" ? V.err : r.kind === "network" ? V.warn : V.dim, title: `分类: ${ERR_LABEL[r.kind] ?? r.kind}` },
						{ text: String(r.count), color: r.count >= 3 ? V.err : r.count === 2 ? V.warn : V.dim, title: `累计 ${r.count} 次` },
						{ text: r.info.length > 160 ? r.info.slice(0, 160) + "…" : r.info, color: V.text, title: r.info, style: { maxWidth: "520px", ...ellipsis } },
						{ text: fmtTs(r.lastAt), color: V.dim, title: `最近出现: ${new Date(r.lastAt).toLocaleString("zh-CN")}` }
					]);
			})();
			// ── 报表③：错误明细（每次一条）——数据源 errorEvents（最近 200 条，滚动）──
			// 排序：按时间倒序（最新在前），方便直接看到最新异常，无需滚动到底部
			const eventToRow = (e) => {
				let info = e.message ?? "";
			...
			const errEventRows = [...(st?.errorEvents || [])]
				.sort((a, b) => (b.at ?? 0) - (a.at ?? 0))
				.map(eventToRow);
			const errEventCount = (st?.errorEvents || []).length;
			// ── 今日范围：从带时间戳的逐条事件过滤"今天 0 点起"的事件，再聚合成今日三份报表 ──
			// 注：errorDetails 是全历史累计；"今日"视图只基于最近事件窗口（≤200 条）内属于今天的部分。
			const startOfToday = (() => { const d = new Date(); d.setHours(0, 0, 0, 0); return d.getTime(); })();
			const todayEvents = (st?.errorEvents || []).filter((e) => (e.at ?? 0) >= startOfToday);
			const buildEventInfo = (e) => {
				let info = e.message ?? "";
				if (e.code && !info.includes(`(code=${e.code})`)) info = `${info} (code=${e.code})`;
				const metaBits = [];
				if (e.status) metaBits.push(`HTTP ${e.status}`);
			...
			const todayEventRows = [...todayEvents].sort((a, b) => (b.at ?? 0) - (a.at ?? 0)).map(eventToRow);
			// 今日：按 分类 × 错误信息 汇总
			const todayKindRows = (() => {
				const kindOrder = { rate_limit: 0, timeout: 1, network: 2, provider: 3, auth: 4, aborted: 5, other: 6, unknown: 7 };
				const agg = new Map();
				for (const e of todayEvents) {
					...
					const info = buildEventInfo(e);
					const key = `${kind}\u0000${info}`;
					const cur = agg.get(key);
					if (cur) { cur.count += 1; if ((e.at ?? 0) > cur.lastAt) cur.lastAt = e.at ?? 0; }
					else agg.set(key, { kind, info, count: 1, lastAt: e.at ?? 0 });
				}
				return [...agg.values()]
					.sort((a, b) =>
						(kindOrder[a.kind] ?? 9) - (kindOrder[b.kind] ?? 9) ||
					...
					.map((r) => [
						{ text: ERR_LABEL[r.kind] ?? r.kind ?? "未知", color: r.kind === "rate_limit" ? V.err : r.kind === "network" ? V.warn : V.dim, title: `分类: ${ERR_LABEL[r.kind] ?? r.kind}` },
						{ text: String(r.count), color: r.count >= 3 ? V.err : r.count === 2 ? V.warn : V.dim, title: `今日累计 ${r.count} 次` },
						{ text: r.info.length > 160 ? r.info.slice(0, 160) + "…" : r.info, color: V.text, title: r.info, style: { maxWidth: "520px", ...ellipsis } },
						{ text: fmtTs(r.lastAt), color: V.dim, title: `最近出现: ${new Date(r.lastAt).toLocaleString("zh-CN")}` }
					]);
			})();
			// 今日：按 会话 × 错误信息 汇总
			const todaySessionRows = (() => {
				const agg = new Map();
				for (const e of todayEvents) {
					const sid = e.sessionId ?? null;
					const sourceKind = e.sourceKind ?? null;
					const kind = e.kind ?? "unknown";
					const info = buildEventInfo(e);
					const key = `${sid ?? "(none)"}\u0000${sourceKind ?? ""}\u0000${kind}\u0000${info}`;
					const cur = agg.get(key);
					if (cur) { cur.count += 1; if ((e.at ?? 0) > cur.lastAt) cur.lastAt = e.at ?? 0; }
					else agg.set(key, { sessionId: sid, sourceKind, kind, info, count: 1, lastAt: e.at ?? 0 });
				}
				return [...agg.values()]
					.sort((a, b) => {
						const ga = a.sessionId ?? "";
					...
					.map((r) => [
						...
						{ text: ERR_LABEL[r.kind] ?? r.kind ?? "未知", color: V.dim },
						{ text: String(r.count), color: r.count >= 3 ? V.err : r.count === 2 ? V.warn : V.dim, title: `今日累计 ${r.count} 次` },
						{ text: r.info.length > 120 ? r.info.slice(0, 120) + "…" : r.info, color: V.text, title: r.info, style: { maxWidth: "440px", ...ellipsis } },
						{ text: fmtTs(r.lastAt), color: V.dim, title: `最近出现: ${new Date(r.lastAt).toLocaleString("zh-CN")}` }
					]);
			})();
			// 异常明细 tab 顶部统计
			...
			// ── 会话并发（sessctl）行渲染：限额控件 + 状态 ──
			const sessRows = (data?.sessions || []).map((s) => {
				const cap = s.cap;
				const limited = !!s.capped;
				const paused = !!s.paused;
				const act = s.active ?? 0;
				const lastAct = s.lastActivityMs || 0;
				const ago = lastAct ? nowMs - lastAct : null;
				const stateCell = act > 0
					? { text: "● 请求中", color: V.ok }
					: paused
						? { text: "⏸ 已暂停", color: V.err }
						: limited
							? { text: `已限 ${cap}`, color: dsw("state-warn-primary", "#eab308") }
							: ago !== null && ago < 60_000
								? { text: "● 在线", color: V.ok }
								: { text: "离线", color: V.dim };
				const dispTitle = s.title || "（无标题）";
				const dispId = s.gateKey ? shortId(s.gateKey) : "无会话";
				const tipLines = [`完整会话 id: ${s.gateKey ?? "无"}`];
				if (s.title) tipLines.push(`标题: ${s.title}`);
				if (s.cwd) tipLines.push(`工作区: ${s.cwd}`);
				return h("tr", { key: s.gateKey, style: { borderBottom: `1px solid ${V.border1}` } },
					h("td", { style: css.td, title: tipLines.join("\n") },
						h("div", { style: { maxWidth: "220px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: V.text, fontWeight: 600 } }, dispTitle),
						h("div", { style: { fontSize: "11px", color: V.dim } }, `id ${dispId}`),
						s.cwd
							? h("div", { style: { fontSize: "11px", color: V.dim } }, `📁 ${baseName(s.cwd)}`)
							: null
					),
					h("td", { style: css.td },
						h("span", { style: { color: kindColor(s.kind) } }, s.kind ? (KIND_LABEL[s.kind] ?? s.kind) : "—")
					),
					h("td", { style: css.td },
						h("span", { style: { color: act > 0 ? V.ok : V.dim, fontWeight: 600 } }, String(act))
					),
					h("td", { style: css.td },
						h("span", { style: { color: (s.sessionWaiting ?? 0) > 0 ? V.warn : V.dim } }, String(s.sessionWaiting ?? 0))
					),
					h("td", { style: css.td },
						h("span", { style: { color: (s.globalWaiting ?? 0) > 0 ? V.warn : V.dim } }, String(s.globalWaiting ?? 0))
					),
					h("td", { style: css.td },
						h("span", { style: { color: stateCell.color } }, stateCell.text)
					),
					h("td", { style: css.td },
						ago !== null
							? h("span", { style: { color: V.dim } }, ago < 60_000 ? `${Math.max(1, Math.round(ago / 1000))}s` : `${Math.max(1, Math.round(ago / 60000))}m`)
					...
					h("td", { style: css.td },
						h("div", { style: { display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" } },
							h("input", {
								...
								min: 0,
								max: 64,
								value: sessionDraft[s.gateKey] ?? (limited ? cap : ""),
								placeholder: limited ? String(cap) : "不限",
								onChange: (e) => setSessionDraft((d) => ({ ...d, [s.gateKey]: e.target.value })),
								style: { width: "52px", boxSizing: "border-box", background: V.panel, color: V.text, border: `1px solid ${V.border}`, borderRadius: "6px", padding: "4px 6px", fontSize: "12px", fontVariantNumeric: "tabular-nums" }
							}),
							h("button", {
								style: css.btnPrimary,
								disabled: busy,
								onClick: () => {
									const v = sessionDraft[s.gateKey];
									patchSession("set", s.gateKey, v === void 0 || v === "" ? cap : v);
								}
							}, "应用"),
							paused
								? h("button", { style: css.btn, disabled: busy, onClick: () => patchSession("resume", s.gateKey) }, "▶ 恢复")
								: h("button", { style: css.btn, disabled: busy, onClick: () => patchSession("pause", s.gateKey) }, "⏸ 暂停"),
							limited
								? h("button", { style: css.btn, disabled: busy, onClick: () => patchSession("clear", s.gateKey) }, "✕ 清除")
								: null
						)
					)
				);
			...
			return h("div", { style: css.wrap },
				h("div", { style: css.header },
					...
					h("div", { style: css.meta },
						...
						h("button", { style: showCfg ? { ...css.btnPrimary } : css.btn, onClick: () => {
							if (!showCfg && cfg) {
								setDraft({
									...
									dayRetention: cfg.dayRetention ?? 0,
									maxStreamStallMin: Math.round((cfg.maxStreamStallMs ?? 600000) / 60000),
									sessionLimitsEnabled: cfg.sessionLimitsEnabled !== false,
									sessionExemptAuxiliary: cfg.sessionExemptAuxiliary !== false,
									sessionLimitTtlDays: cfg.sessionLimitTtlDays ?? 0
								});
							}
							setShowCfg((v) => !v);
						} }, showCfg ? "✕" : "⚙ 配置"),
						...
						h("button", { style: css.btn, onClick: () => setShowClean(true) }, "🗑 清历史"),
				...
				showCfg && draft ? h("div", { style: css.modalOverlay, onClick: () => setShowCfg(false) },
					h("div", { style: css.modalCard, onClick: (e) => e.stopPropagation() },
						...
						h("div", { style: { borderTop: `1px solid ${V.border1}`, paddingTop: "14px", marginTop: "6px" } },
							h("div", { style: { fontSize: "12px", color: V.dim, marginBottom: "8px" } }, "会话级并发控制（v1.5.0+）"),
							cfgCheck("启用会话级限制", "关闭后会话门完全跳过（已设限额保留但惰性生效），只走全局门", draft.sessionLimitsEnabled, (v) => setDraft({ ...draft, sessionLimitsEnabled: v })),
							cfgCheck("辅助请求豁免", "压缩 / 标题请求只过全局门，不被会话限流拖死", draft.sessionExemptAuxiliary, (v) => setDraft({ ...draft, sessionExemptAuxiliary: v })),
							cfgField("限额过期", "天", "会话限额条目超过该天数未再使用自动清除；0 = 永久保留", draft.sessionLimitTtlDays, (v) => setDraft({ ...draft, sessionLimitTtlDays: v }))
						),
							...
							h("button", { style: css.btnPrimary, disabled: cfgBusy, onClick: async () => {
								...
								try {
									await patchConfig({
										...
										dayRetention: Math.max(0, Math.floor(Number(draft.dayRetention) || 0)),
										maxStreamStallMs: Math.max(1000, Math.floor(Number(draft.maxStreamStallMin) || 10) * 60000),
										sessionLimitsEnabled: !!draft.sessionLimitsEnabled,
										sessionExemptAuxiliary: !!draft.sessionExemptAuxiliary,
										sessionLimitTtlDays: Math.max(0, Math.floor(Number(draft.sessionLimitTtlDays) || 0))
									});
									setShowCfg(false);
								} finally {
									setCfgBusy(false);
				...
				showClean ? h("div", { style: css.modalOverlay, onClick: () => setShowClean(false) },
					h("div", { style: css.modalCard, onClick: (e) => e.stopPropagation() },
						h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" } },
							h("div", null,
								h("div", { style: { fontSize: "15px", fontWeight: 700, color: V.text } }, "🗑 历史数据清理"),
								h("div", { style: { fontSize: "11px", color: V.dim, marginTop: "2px" } }, "按保留天数裁剪历史；只影响勾选的数据块，保存后立即生效并持久化")
							),
							h("button", { style: css.modalClose, onClick: () => setShowClean(false), title: "关闭" }, "✕")
						),
						h("div", { style: { marginBottom: "14px" } },
							h("div", { style: { fontSize: "13px", color: V.text, fontWeight: 600, marginBottom: "3px" } }, "保留最近"),
							h("div", { style: { fontSize: "11px", color: V.dim, lineHeight: "1.5", marginBottom: "6px" } }, "只保留最近 N 天的数据（含今天），更早的记录将被删除；0 = 清空全部历史数据"),
							h("div", { style: { display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "8px" } },
								[3, 7, 30, 90].map((n) =>
									h("button", { key: n, style: cleanDraft.keepDays === n ? css.btnPrimary : css.btn, onClick: () => setCleanDraft({ ...cleanDraft, keepDays: n }) }, `${n} 天`)
								),
								h("input", {
									type: "number", min: "0", placeholder: "自定义…",
									value: [3, 7, 30, 90].includes(cleanDraft.keepDays) ? "" : cleanDraft.keepDays,
									onChange: (e) => setCleanDraft({ ...cleanDraft, keepDays: Math.max(0, Math.floor(Number(e.target.value) || 0)) }),
									style: { width: "90px", background: V.panel, color: V.text, border: `1px solid ${V.border}`, borderRadius: "6px", padding: "6px 10px", fontSize: "13px" }
								})
							),
							h("div", { style: { fontSize: "11px", color: V.dim } }, "提示：0 = 清空全部；设为 3 即只保留最近 3 天")
						),
						h("div", { style: { marginBottom: "14px", borderTop: `1px solid ${V.border1}`, paddingTop: "12px" } },
							h("div", { style: { fontSize: "13px", color: V.text, fontWeight: 600, marginBottom: "6px" } }, "清理范围"),
							[
								["days", "每日汇总（byDay）", "每日请求/异常等汇总，仅保留最近 N 天"],
								["details", "异常聚合（errorDetails）", "按分类×错误信息聚合，仅保留最近还活跃的错误"],
								["events", "逐条事件（errorEvents）", "逐条错误明细，仅保留最近 N 天内的事件"],
								["totals", "全历史累计（totals）", "按保留期内汇总重算，不再是自安装以来的全量累计"]
							].map(([key, label, desc]) =>
								h("div", { key, style: { marginBottom: "10px", display: "flex", gap: "10px", alignItems: "flex-start" } },
									h("input", {
										type: "checkbox",
										checked: !!cleanDraft.scope[key],
										onChange: (e) => setCleanDraft({ ...cleanDraft, scope: { ...cleanDraft.scope, [key]: e.target.checked } }),
										style: { marginTop: "2px", accentColor: "var(--dsw-state-business-primary, #3b82f6)" }
									}),
									h("div", { style: { flex: 1 } },
										h("div", { style: { fontSize: "13px", color: V.text, fontWeight: 600, marginBottom: "2px" } }, label),
										h("div", { style: { fontSize: "11px", color: V.dim, lineHeight: "1.5" } }, desc)
									)
								)
							)
						),
						h("div", { style: { display: "flex", gap: "8px", alignItems: "center", borderTop: `1px solid ${V.border1}`, paddingTop: "14px" } },
							h("button", { style: css.btnPrimary, disabled: cfgBusy, onClick: async () => {
								const scopes = Object.keys(cleanDraft.scope).filter((k) => cleanDraft.scope[k]);
								if (scopes.length === 0) { setError("请至少勾选一个清理范围"); return; }
								setCfgBusy(true);
								try {
									await patchHistory("retain", { keepDays: cleanDraft.keepDays, scope: scopes });
									setShowClean(false);
								} finally {
									setCfgBusy(false);
								}
							} }, cfgBusy ? "清理中…" : "确认清理"),
							h("button", { style: css.btn, onClick: () => setShowClean(false) }, "取消"),
							h("span", { style: { fontSize: "11px", color: V.dim, marginLeft: "auto" } }, "也可在配置弹窗里设「历史保留天数」自动裁剪")
						)
					)
				) : null,
				...
				tab === "errors" ? h("div", { style: css.section },
					...
					h("div", { style: { ...css.tabBar, marginBottom: "8px" } },
						h("span", { style: { fontSize: "12px", color: V.dim, marginRight: "6px" } }, "范围："),
						[["today", "今日"], ["all", "全部"]].map(([key, label]) =>
							h("button", {
								key,
								style: errScope === key ? { ...css.tabBtn, ...css.tabBtnActive } : css.tabBtn,
								onClick: () => setErrScope(key)
							}, label)
						)
					),
					h("div", { style: css.tabBar },
						[["kind", "分类汇总"], ["session", "按会话汇总"], ["detail", "逐条明细"]].map(([key, label]) =>
							h("button", {
								key,
								style: errTab === key ? { ...css.tabBtn, ...css.tabBtnActive } : css.tabBtn,
								onClick: () => setErrTab(key)
							}, label)
						)
					),
					errTab === "kind" ? h("div", { style: css.section },
						h("h3", { style: css.sectionTitle },
							errScope === "today"
								? `按 分类 × 错误信息 汇总（今日 · 当天发生 · 分类内按次数降序 · 最多 100 行）`
								: `按 分类 × 错误信息 汇总（全历史累计 · 分类内按次数降序 · 最多 100 行）`),
						table(["分类", "次数", "错误信息", "最近出现"],
							errScope === "today" ? todayKindRows : errKindRows,
							errScope === "today"
								? "今日暂无异常记录（当天发生过的异常会按 分类×错误信息 聚合在此）"
								: "暂无异常记录（发生过的异常会按 分类×错误信息 聚合在此，含错误码/HTTP status/requestId）")
					) : null,
					errTab === "session" ? h("div", { style: css.section },
						h("h3", { style: css.sectionTitle },
							errScope === "today"
								? `① 错误汇总（按 会话 × 错误信息 分组 · 今日 · 当天发生 · 最多 50 行）`
								: `① 错误汇总（按 会话 × 错误信息 分组 · 全历史累计 · 最多 50 行）`),
						table(["会话", "来源", "分类", "次数", "错误信息", "最近出现"],
							errScope === "today" ? todaySessionRows : errSummaryRows,
							errScope === "today"
								? "今日暂无异常记录（当天发生过的异常会按 会话×错误 聚合在此）"
								: "暂无异常记录（发生过的异常会按 会话×错误 聚合在此，含错误码/HTTP status/requestId）")
					) : null,
					errTab === "detail" ? h("div", { style: css.section },
						h("h3", { style: css.sectionTitle },
							errScope === "today"
								? `② 错误明细（每次错误一条 · 今日 ${todayEventRows.length} 条 · 最新在前 · 倒序）`
								: `② 错误明细（每次错误一条 · 最近 ${errEventCount} / 200 条滚动窗口 · 最新在前 · 倒序）`),
						table(["会话", "来源", "分类", "详细错误信息", "时间"],
							errScope === "today" ? todayEventRows : errEventRows,
							errScope === "today"
								? "今日暂无明细（逐条事件自 v1.3.9 起采集）"
								: "暂无明细（逐条事件自 v1.3.9 起采集：重启宿主后新错误才会逐条记录；旧错误仅汇总无明细）")
					) : null
				) : null,
				...
				tab === "sessctl" ? h("div", { style: css.section },
					h("h3", { style: css.sectionTitle }, "会话并发（按顶层会话聚合 · 含其全部子代理）"),
					h("div", { style: { fontSize: "11px", color: V.dim, marginBottom: "8px", lineHeight: "1.6" } },
						`总开关：${cfg?.sessionLimitsEnabled === false ? "关闭（仅全局门）" : "开启"} ｜ 辅助请求豁免：${cfg?.sessionExemptAuxiliary === false ? "否" : "是"} ｜ 全局上限 ${max} ｜ cap=0 = 暂停（fail-open 仍兜底）`
					),
					h("div", { style: { overflowX: "auto" } },
						h("table", { style: css.table },
							h("thead", null, h("tr", null,
								["会话", "来源", "活跃", "会话门等待", "全局等待", "状态", "最近活动", "限额操作"].map((hd, i) => h("th", { key: i, style: css.th }, hd))
							)),
							h("tbody", null,
								sessRows.length
									? sessRows
									: h("tr", null, h("td", { colSpan: 8, style: { ...css.td, color: V.dim } }, "暂无在线会话（有模型请求后自动出现；限额可随时手动添加）"))
							)
						)
					)
				) : null,
