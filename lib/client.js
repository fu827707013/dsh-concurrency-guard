		...
		function ConcurrencyPanel() {
			...
			const errSummaryRows = (() => {
				...
				for (const [key, d] of Object.entries(st?.errorDetails ?? {})) {
					...
					if (!sessEntries.length) {
						...
						rows.push({ sessionId: null, kind: d.kind ?? "unknown", info, hasCode, count: d.count ?? 1, lastAt: d.lastAt ?? 0 });
					} else {
						for (const [sid, sc] of sessEntries) {
							...
							rows.push({ sessionId: sid === "(none)" ? null : sid, kind: d.kind ?? "unknown", info, hasCode, count: c, lastAt });
						}
					}
				}
				return rows
					...
					.map((r) => [
						...
						{ text: r.info.length > 120 ? r.info.slice(0, 120) + "…" : r.info, color: r.hasCode ? dsw("state-warn-primary", "#eab308") : V.text, title: r.info, style: { maxWidth: "460px", ...ellipsis } },
			...
			const errEventRows = (() => {
				...
				return ordered.map((e) => {
					...
					return [
						...
						{ text: info.length > 200 ? info.slice(0, 200) + "…" : info, color: e.code ? dsw("state-warn-primary", "#eab308") : V.text, title, style: { maxWidth: "560px", ...ellipsis } },
			...
			return h("div", { style: css.wrap },
				...
				tab === "errors" ? h("div", { style: css.section },
					...
					table(["会话", "分类", "次数", "错误信息", "最近出现"], errSummaryRows, "暂无异常记录（发生过的异常会按 会话×错误 聚合在此，含错误码/HTTP status/requestId）"),
					h("h3", { style: { ...css.sectionTitle, marginTop: "16px" } }, `② 错误明细（每次错误一条 · 按会话+时间排序 · 最近 ${errEventCount} / 200 条滚动窗口）`),
					table(["会话", "分类", "详细错误信息", "时间"], errEventRows, "暂无明细（逐条事件自 v1.3.9 起采集：重启宿主后新错误才会逐条记录；旧错误仅汇总无明细）")
				) : null,
