...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const cfgField = (label, value, onChange, unit) =>
			h("label", { style: { display: "flex", flexDirection: "column", gap: "4px", fontSize: "12px", color: V.dim } },
				h("span", null, `${label}（${unit}）`),
				h("input", {
					type: "number",
					value: value === null || value === void 0 ? "" : value,
					onChange: (e) => onChange(e.target.value),
					style: { background: V.panel, color: V.text, border: `1px solid ${V.border}`, borderRadius: "6px", padding: "4px 8px", fontSize: "13px", fontVariantNumeric: "tabular-nums" }
				})
			);
		...
		function ConcurrencyPanel() {
			...
			return h("div", { style: css.wrap },
				...
				showCfg && draft ? h("div", { style: { ...css.card, marginBottom: "12px" } },
					h("div", { style: { display: "flex", justifyContent: "space-between", fontSize: "12px", color: V.dim, marginBottom: "8px" } },
						h("span", null, "⚙ 配置（保存后立即生效并持久化，重启保留；mode 用右上角按钮切换）"),
						h("span", null, `当前 mode: ${mode === "queue" ? "排队节制" : "仅监控"}`)
					),
					h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))", gap: "10px" } },
						cfgField("并发上限", draft.maxConcurrency, (v) => setDraft({ ...draft, maxConcurrency: v }), "个"),
						cfgField("告警阈值", draft.warnAt, (v) => setDraft({ ...draft, warnAt: v }), "个"),
						cfgField("排队超时", draft.maxQueueWaitSec, (v) => setDraft({ ...draft, maxQueueWaitSec: v }), "秒"),
						cfgField("历史条数", draft.history, (v) => setDraft({ ...draft, history: v }), "条"),
						cfgField("历史TTL", draft.historyTtlMin, (v) => setDraft({ ...draft, historyTtlMin: v }), "分钟"),
						cfgField("停滞判死", draft.maxStreamStallMin, (v) => setDraft({ ...draft, maxStreamStallMin: v }), "分钟")
					),
					h("div", { style: { display: "flex", gap: "8px", marginTop: "10px", alignItems: "center" } },
						...
						h("span", { style: { fontSize: "11px", color: V.dim } }, "保存即写入 state.json，重启不丢；也可在 Settings → Plugins → Plugin configuration 编辑")
					)
				) : null,
