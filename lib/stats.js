...
function mergeDay(a, b) {
  const out = { ...a, started: a.started + b.started, completed: a.completed + b.completed, cancelled: a.cancelled + b.cancelled, errored: a.errored + b.errored, gateHeld: (a.gateHeld ?? 0) + (b.gateHeld ?? 0), failOpen: (a.failOpen ?? 0) + (b.failOpen ?? 0), byKind: { ...a.byKind }, byErrKind: { ...a.byErrKind } };
...
export function adoptStats(raw) {
  ...
  const sanitizeDay = (d) => {
    ...
    for (const k of ["started", "completed", "cancelled", "errored", "gateHeld", "failOpen", "warnHits"]) base[k] = num(d[k]);
