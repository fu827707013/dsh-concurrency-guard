 ...
 */
export function recordFinish(stats, status, errKind, kind, now = Date.now(), errorInfo, dayRetention = 0) {
  // 跨天回滚：把 today 归档进 byDay，开新桶；若配置了保留天数则顺带裁剪历史
  ...
  if (key !== stats.todayKey) {
    ...
    if (dayRetention > 0) retainStats(stats, dayRetention, now);
  }
  const t = stats.today;
...
function mergeDay(a, b) {
  const out = { ...a, started: a.started + b.started, completed: a.completed + b.completed, cancelled: a.cancelled + b.cancelled, interrupted: (a.interrupted ?? 0) + (b.interrupted ?? 0), errored: a.errored + b.errored, gateHeld: (a.gateHeld ?? 0) + (b.gateHeld ?? 0), failOpen: (a.failOpen ?? 0) + (b.failOpen ?? 0), sessionGateHeld: (a.sessionGateHeld ?? 0) + (b.sessionGateHeld ?? 0), sessionFailOpen: (a.sessionFailOpen ?? 0) + (b.sessionFailOpen ?? 0), byKind: { ...a.byKind }, byErrKind: { ...a.byErrKind } };
...
export function adoptStats(raw) {
  ...
  const sanitizeDay = (d) => {
    ...
    for (const k of ["started", "completed", "cancelled", "interrupted", "errored", "gateHeld", "failOpen", "sessionGateHeld", "sessionFailOpen", "warnHits"]) base[k] = num(d[k]);
...
/**
 * 历史数据保留裁剪：只保留最近 keepDays 天（含今天）产生的数据。
 * - byDay        删掉日期 < cutoff 的天
 * - errorDetails 只保留最近还出现过的错误键（lastAt >= cutoffMs）
 * - errorEvents  删掉 at < cutoffMs 的逐条事件
 * - totals       重算为保留期内 byDay 之和 + 今天（不再是"自安装以来"的全历史累计）
 * keepDays = 0 表示清空全部历史（保留今天内存计数不变）。
 * scope（可选）控制清理哪些块，默认全部：["days","details","events","totals"]。
 * @param {object} stats
 * @param {number} keepDays - 保留天数（0 = 清空全部历史数据）
 * @param {number} [now]
 * @param {string[]} [scope]
 * @returns {{stats: object, removed: {days:number, details:number, events:number}}}
 */
export function retainStats(stats, keepDays, now = Date.now(), scope = ["days", "details", "events", "totals"]) {
  const removed = { days: 0, details: 0, events: 0 };
  const cleanAll = !keepDays || keepDays <= 0;
  const doDays = scope.includes("days");
  const doDetails = scope.includes("details");
  const doEvents = scope.includes("events");
  const doTotals = scope.includes("totals");
  let cutoffMs = now;
  let cutoffDayKey = null;
  if (!cleanAll) {
    const start = new Date(now);
    start.setHours(0, 0, 0, 0);
    cutoffMs = start.getTime() - (keepDays - 1) * 86_400_000;
    cutoffDayKey = dayKey(cutoffMs);
  }

  // 1. byDay：保留日期 >= cutoff 的天（或全清）
  if (doDays) {
    if (cleanAll) {
      removed.days = Object.keys(stats.byDay ?? {}).length;
      stats.byDay = {};
    } else {
  ...
  // 2. errorDetails：只保留最近还活跃的错误键
  if (doDetails) {
    if (cleanAll) {
      removed.details = Object.keys(stats.errorDetails ?? {}).length;
      stats.errorDetails = {};
    } else {
      for (const [k, v] of Object.entries(stats.errorDetails ?? {})) {
        if ((v?.lastAt ?? 0) < cutoffMs) {
          delete stats.errorDetails[k];
          removed.details += 1;
        }
      }
    }
  ...
  // 3. errorEvents：删掉早于 cutoff 的事件
  if (doEvents) {
    if (cleanAll) {
      removed.events = (stats.errorEvents ?? []).length;
      stats.errorEvents = [];
    } else {
      const before = (stats.errorEvents ?? []).length;
      stats.errorEvents = (stats.errorEvents ?? []).filter((e) => (e?.at ?? 0) >= cutoffMs);
      removed.events = before - stats.errorEvents.length;
    }
  }

  // 4. totals 重算：保留期内 byDay 之和 + 今天（若也清 days/events，则按保留期口径）
  if (doTotals) {
    let merged = EMPTY_DAY();
    for (const [k, v] of Object.entries(stats.byDay ?? {})) {
      if (cleanAll || k >= cutoffDayKey) merged = mergeDay(merged, v);
    }
    merged = mergeDay(merged, stats.today);
    stats.totals = merged;
  }
  // firstSeenAt 对齐到保留期内最早一天（近似，仅展示用；仅当清理了 byDay 时）
  if (!cleanAll && doDays) {
    const days = Object.keys(stats.byDay ?? {}).sort();
    if (days.length > 0) stats.firstSeenAt = new Date(`${days[0]}T00:00:00`).getTime();
  }
