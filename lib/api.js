...
import { retainStats } from "./stats.js";
...
export function createService(state, logger) {
  return {
    ...
    /**
     * 历史数据保留裁剪：只保留最近 keepDays 天（含今天）的数据。
     * @param {number} keepDays - 保留天数（0 = 清空全部历史数据）。
     * @param {string[]} [scope] - 清理哪些块，默认全清：["days","details","events","totals"]。
     * @returns {{removed: {days:number, details:number, events:number}, totals: object}}
     */
    retainHistory(keepDays, scope) {
      const days = Math.max(0, Math.floor(Number(keepDays) || 0));
      const sc = Array.isArray(scope) && scope.length > 0
        ? scope.filter((s) => ["days", "details", "events", "totals"].includes(s))
        : ["days", "details", "events", "totals"];
      if (sc.length === 0) throw new Error("scope 不能为空，可选：days / details / events / totals");
      const before = Object.keys(state.stats.byDay ?? {}).length;
      const { stats, removed } = retainStats(state.stats, days, Date.now(), sc);
      state.stats = stats;
      // 若清了 byDay 或 totals，把内存计数也对齐到今天（today 未动，计数语义不变）
      logger.info(
        `[concurrency-guard] 历史保留清理 keepDays=${days} scope=[${sc.join(",")}]：` +
          `每日 ${removed.days} 天 / 异常聚合 ${removed.details} 条 / 逐条事件 ${removed.events} 条（byDay ${before}→${Object.keys(stats.byDay ?? {}).length}）`
      );
      state.onChange?.();
      return { removed, totals: stats.totals, byDayCount: Object.keys(stats.byDay ?? {}).length };
    },
        ...
        handler: async (req, res) => {
          try {
            ...
            if (action === "clear") {
              ...
              sendJson(res, 200, getApi().retainHistory(body?.keepDays, body?.scope));
            } else {
              sendJson(res, 400, { error: "action 必须是 clear / prune / reset / retain（retain 需带 keepDays，可带 scope）" });
            }
          } catch (error) {
            sendJson(res, 400, { error: String(error?.message ?? error) });
          }
...
/** 惰性注册 concurrency_status / concurrency_session_list / concurrency_session_set 工具（tools 服务就绪后才挂载）。 */
export function registerTool(ctx, state, logger, getApi) {
  let didTool = false;
